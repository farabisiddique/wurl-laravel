      ...
      <form action="" method="POST" class="mb-4">
        ...
        <div class="input-group mb-3">
            <input type="text" class="form-control formInput" placeholder="Paste your long link here." 
                aria-label="Paste your long link here." id="longLinkInput" required>
        ...
        <div class="input-group mb-3">
            ...
            <select class="form-select formInput" aria-label="Domain selection" id="domainSelect" onchange="checkCustomText()">
                @foreach ($domains as $domain)
                    <option value="{{ $domain->id }}">{{ $domain->domain_name   }}</option>
                @endforeach
            </select>
            <span class="input-group-text">/</span>
            <input type="text" class="form-control formInput" placeholder="Custom text" aria-label="Custom text" id="customTextInput">
        </div>
